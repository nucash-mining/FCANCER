...
contract WALTBridgeCrossChain is Ownable, ReentrancyGuard, Pausable {
    ...
    constructor(address _waltToken, address _messenger) {
    ...
    // Emergency function to handle stuck tokens
    function emergencyBridgeComplete(
        address user,
        uint256 amount,
        bytes32 bridgeId
    ) external onlyOwner {
        ...
        emit BridgeCompleted(user, amount, 0, bridgeId, msg.sender);
    }
}